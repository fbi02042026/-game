...
public class EquipDropPopupUI : MonoBehaviour
{
    ...
    void OnPrimary()
    {
        ...
        // 装备不进背包（2026-09-15）：直接穿上，被替换下来的旧件自动分解
        if (!bag.TryEquipDirect(sel))
        {
    ...
    void OnSecondary()
    {
        ...
        // 装备不进背包（2026-09-15）：不穿 = 直接分解换强化石，不做二次确认
        if (sel != null && bag != null)
        {
            bag.ScrapEquip(sel);
            BattleUI.Instance?.UpdateBackpackGrid();
        }
    ...
    void OnClose()
    {
        // 关闭 = 不穿。装备不进背包，所以也直接分解（不再有「留在背包」这条路）
        var sel = GetSelected();
        if (sel != null)
        {
            GridBackpackSystem.Instance?.ScrapEquip(sel);
            BattleUI.Instance?.UpdateBackpackGrid();
        }
        Finish(sel, false);
    }
