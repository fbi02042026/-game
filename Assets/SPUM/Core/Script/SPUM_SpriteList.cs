...
public class SPUM_SpriteList : MonoBehaviour
{
    ...
    public void LoadSprite(SPUM_SpriteList data)
    {
        ...
        if (_eyeList.Count > 2 && data._eyeList.Count > 2)
        {
            ...
            if (_eyeList.Count > 3 && data._eyeList.Count > 3)
                _eyeList[3].color = data._eyeList[3].color;
        }

        if (_hairList.Count > 3 && data._hairList.Count > 3)
        {
        ...
        else if (_hairList.Count > 0 && data._hairList.Count > 0)
        {
            _hairList[0].color = data._hairList[0].color;
            _hairList[0].gameObject.SetActive(!data._hairList[0].gameObject.activeInHierarchy);
        }
    ...
    {
        if (_objList == null || _pathList == null) return;
        for (var i = 0; i < _pathList.Count; i++)
        {
            if (i >= _objList.Count) break;
            if (_pathList[i] == null || _pathList[i].Length <= 1) continue;
            ...
            if (tSP.Length > 1)
            {
                if (_objList[0] != null && (_objList[0].name == "ClothBody" || _objList[0].name == "BodyArmor"))
                {
                    string tmpName = "";
                    switch (i)
                    {
