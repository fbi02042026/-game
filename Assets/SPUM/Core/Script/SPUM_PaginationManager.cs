...
public partial class SPUM_PaginationManager : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        {
            Debug.LogError("[SPUM] PaginationManager Start() 失败: " + e.Message);
        }
    }
    ...
    void LoadItems()
    {
        ...
        if (SPUM_Manager == null || SPUM_Manager.fileHandler == null)
        {
            Debug.LogWarning("[SPUM] PaginationManager: SPUM_Manager 或 fileHandler 为 null，跳过加载预制体。");
            return;
        }

        var SavedPrefabs = SPUM_Manager.fileHandler.Load(); // Resources.LoadAll<SPUM_Prefabs>("");
        if (SavedPrefabs == null) return;
    ...
    void DisplayPage()
    {
        ...
        {
            Debug.LogWarning("[SPUM] PaginationManager: contentParent 为 null，跳过页面显示。请在 Inspector 中赋值。");
            return;
        }
